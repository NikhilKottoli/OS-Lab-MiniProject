...
import './scheduling.css'; // Make sure you have the scheduling.css file in the same directory
...
const CPUScheduling = () => {
  return (
    <div style={{ marginLeft: '1%' }}>
      ...
      <button onClick={() => window.location.href = 'FCFS/fcfs.html'}>Simulate</button>
      ...
      <button onClick={() => window.location.href = 'SJF/sjf.html'}>Simulate</button>
      ...
      <button onClick={() => window.location.href = 'PRIORITY/priority.html'}>Simulate</button>

      <h2>Round Robin</h2>
      <p>
        The most efficient CPU Process Scheduling Algorithm named Round Robin CPU Process Scheduling. This algorithm is very special because it is going to remove all the Flaws which we have detected in the previous CPU Process Scheduling Algorithms. 
        There is a lot of popularity for this Round Robin CPU Scheduling because Round Robin works only in Pre Emptive state. This makes it very reliable.
      </p>
      <h4>Advantages:</h4>
      <ul>
      ...
      <button onClick={() => window.location.href = 'RR/round-robin.html'}>Simulate</button>

      <h2>SRTF</h2>
      <p>
        The process with the smallest amount of time remaining until completion is selected to execute. Since the currently executing process is the one with the shortest amount of time remaining by definition, and since that time should only reduce as execution progresses, 
        processes will always run until they complete or a new process is added that requires a smaller amount of time.
      </p>
      <h4>Advantages:</h4>
      <ul>
        <li>SRTF algorithm makes the processing of the jobs faster than SJN algorithm, given its overhead charges are not counted.</li>
      ...
      <h4>Disadvantages:</h4>
      <ul>
        <li>The context switch is done a lot more times in SRTF than in SJN, and consumes CPU’s valuable time for processing. This adds up to its processing time and diminishes its advantage of fast processing.</li>
      ...
      <button onClick={() => window.location.href = 'SRTF/srtf.html'}>Simulate</button>

      <h2>LJF</h2>
      <p>
        This algorithm is based on the burst time of the processes. The processes are put into the ready queue based on their burst times i.e., in descending order of the burst times. As the name suggests, this algorithm is based on the fact that the process with the largest burst time is processed first. 
      ...
      <button onClick={() => window.location.href = 'LJF/ljf.html'}>Simulate</button>
    </div>
  );
};
